...
| 6 | ML/AI/apps & retire Rust | 🟡 in progress — `code-repository-review-service` ✅ foundation; `media-transform-runtime-service` ✅ foundation; `entity-resolution-service` ✅ architecture+1-vertical-slice; `ontology-exploratory-analysis-service` ✅ substrate-only (matches Rust which mounts only `/health` + `/readiness` until 4 consolidation merges land — timeseries/time-series-data/geospatial-intelligence/scenario-simulation; migrations + typed model scaffolding ship now). Inventory at openfoundry-go/INVENTORY-PHASE6.md. AI-kernel-bound shells blocked on libs/ai-kernel-go (~7515 LOC, multi-iteration). Architecture+slice candidates queued: reindex-coordinator, lineage, workflow-automation, federation-product-exchange. pyo3 sidecars (notebook-runtime, pipeline-build, ontology-actions) STOP-and-ask. |
