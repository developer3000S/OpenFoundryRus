...
| 6 | ML/AI/apps & retire Rust | 🟡 in progress — `code-repository-review-service` ✅ foundation (global-branching CRUD + outbox `foundry.global.branch.promote.requested.v1` + Postgres SubscriberPort for `foundry.branch.events.v1`; code-security tables migrated, scanner integration deferred). Inventory at openfoundry-go/INVENTORY-PHASE6.md. AI-kernel-bound shells (llm-catalog, model-{catalog,deployment}, agent-runtime, ai-evaluation, application-composition, solution-design, retrieval-context) blocked on libs/ai-kernel-go (~7515 LOC, multi-iteration). Architecture+slice candidates queued: media-transform-runtime, entity-resolution, ontology-exploratory-analysis, reindex-coordinator, lineage, workflow-automation, federation-product-exchange. pyo3 sidecars (notebook-runtime, pipeline-build, ontology-actions) STOP-and-ask. |
