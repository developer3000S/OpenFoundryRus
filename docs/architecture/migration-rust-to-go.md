...
| 1.5 | Tier-2 libs (cassandra-kernel, authz-cedar, saga, state-machine, scheduling, search/storage/vector abstractions, geospatial, media-scanner, ontology-kernel, pipeline-expression, plugin-sdk, analytical-logic, ai-kernel, ml-kernel) | 🟡 in progress — `libs/authz-cedar-go` ✅ feature-complete; `libs/ai-kernel-go` ✅ models (7/7) + domain/{llm cache+gateway+guardrails+provider+**runtime full**, agents memory+planner+**executor full**, rag full, evaluation, copilot} + handlers (5/5: tools/prompts/knowledge/agents/chat — agents/execute now fully wired through executor + runtime, chat completion+copilot+benchmark still 501-stubbed pending ai_semantic_cache + ai_conversations + ai_llm_usage_events DB-side-effect helpers); `libs/ml-kernel-go` ✅ models (10/10) + domain/{drift, predictions, training/{hyperparameter,runner,execute_training}, **interop full**} + handlers (7/7: overview/predictions/training/features/deployments/models/experiments — training and models version registration now fully wired through interop + execute_training, runs CRUD + asset-lineage still 501-stubbed pending their own slices). **Zero pure-logic modules pending in either kernel lib**; remaining work is DB-side-effect helpers + auth-middleware integration. Other tier-2 libs migrate alongside first consuming service. |
...
| 6 | ML/AI/apps & retire Rust | 🟡 in progress — all 8 architecture+slice / foundation / substrate service ports done: `code-repository-review-service` ✅, `media-transform-runtime-service` ✅, `entity-resolution-service` ✅, `ontology-exploratory-analysis-service` ✅, `reindex-coordinator-service` ✅, `lineage-service` ✅, `workflow-automation-service` ✅, `federation-product-exchange-service` ✅. **`libs/ai-kernel-go`** ✅ full handler surface + every pure-logic domain (llm including runtime + agents including executor + rag + evaluation + copilot). **`libs/ml-kernel-go`** ✅ full handler surface + every pure-logic domain (drift + predictions + training including runner+execute_training + interop). 16 substrate-only or full-foundation shells all bind the kernel handlers today. agent-runtime/execute now fully wires through executor + llm runtime; training-jobs and model-version registration fully wire through interop + execute_training. Remaining 501 stubs are blocked on DB-side-effect helpers (chat completion ai_semantic_cache + ai_conversations + ai_llm_usage_events), auth-middleware-go purpose-checkpoint integration (chat completion + sensitive agent tools), and the isolated runs CRUD + asset-lineage scaffolding in handlers/experiments.go; pyo3 sidecars STOP-and-ask. |
